...
PID_type PID_Init(PID_type pidstruct)
{
	...
	pidstruct.Target_val = 0;//单位时间内编码器值
	pidstruct.Err = 0;
	pidstruct.LastErr = 0;
	pidstruct.PrevErr = 0;
	pidstruct.Sum_err = 0;
...
PID_type PID_control(PID_type pidstruct,int16_t speed)
{
  ...
  pidstruct.Err = pidstruct.Target_val - speed;   
	...
	if(pidstruct.Sum_err > 399)pidstruct.Sum_err = 399;
	if(pidstruct.Sum_err < -399)	 pidstruct.Sum_err = -399;//0
	...
	if(pidstruct.Output_val > 399)
        pidstruct.Output_val = 399;
    if(pidstruct.Output_val < -399)
        pidstruct.Output_val = -399;
	//}
